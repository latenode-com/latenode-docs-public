...
<Callout type="info" title="No provider API key?">
  Use a **Plug and Play** app node from the catalog: you do not enter a provider key there (billing is PnP). For OpenAI models, see [**ChatGPT**](/integrations/app-nodes/chatgpt) (**Send Message to ChatGPT**).
</Callout>
...
Connection plus **chat input**: message text, file, and/or OpenAI-style **history** JSON.
...
Standard **sampling** parameters for your provider's OpenAI-compatible API.
...
**Structured Output** and **JSON Schema** when the model supports them.
...
**Function calling** when your provider and model support tools.
