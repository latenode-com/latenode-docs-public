...
Pick the **OpenRouter model**: manual **Model ID**, list filters, or **Auto Router**.
...
**Prompt**, optional **file**, and/or **message history** for the selected model.
...
OpenRouter **generation** controls: temperature, top-p/k, max tokens, penalties, stops, bias, compression.
...
**Structured output**, JSON schema, GBNF grammar, and optional **logprobs**.
...
**Function definitions** and how the model must pick a tool.
...
**Reasoning chain** options: effort, summary, token budget, hide from output.
...
Prompt **cache** settings, TTL, and **Session ID** for observability.
