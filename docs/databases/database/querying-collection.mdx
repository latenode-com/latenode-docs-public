...
This page is a **YAML format reference** for the **Filter** field in **Get elements** and **Update objects**. If you are new to the built-in database in the builder, start with [Database essentials](./database-basics) for a quick start and a full walkthrough with a sample client record.
