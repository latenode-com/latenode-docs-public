...
This page is a **YAML format reference** for the **Filter** field in **Get elements** and **Update objects**. If you are new to the built-in database in the builder, start with [Working with the built-in database](./database-basics) for a quick start and a full walkthrough with a sample client record.
