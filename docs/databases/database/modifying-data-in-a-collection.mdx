...
This page documents the **YAML modifier format** for the **Updater** field in **Update objects** (the `items` key). For a step-by-step tour of every database node with a concrete object example, see [Database essentials](./database-basics).
