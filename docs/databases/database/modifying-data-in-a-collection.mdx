...
This page documents the **YAML modifier format** for the **Updater** field in **Update objects** (the `items` key). For a step-by-step tour of every database node with a concrete object example, see [Working with the built-in database](./database-basics).
...
- apply a set of filters to select multiple objects (see [querying a collection](./querying-collection))
